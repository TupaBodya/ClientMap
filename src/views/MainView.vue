<template>
  <div class="main-container" :class="theme">
    ...
    <!-- Уведомление о множественных результатах -->
      ...
      <div class="floor-controls glassmorphism">
        <button 
          ...
          :class="{ 
            ...
            'pulse-highlight': floorsWithSearchResults.includes(floor) && searchResults.length > 1 
          }"
          @click="selectFloor(floor)"
          class="floor-btn"
        >
          {{ floor }} этаж
          <span v-if="floorsWithSearchResults.includes(floor) && searchResults.length > 1" class="result-badge">
            {{ getResultsCountOnFloor(floor) }}
          </span>
        </button>
      </div>
      ...
      <div class="map-content" ref="mapContent" :style="{ backgroundImage: `url(${currentMapImage})` }">
        <svg id="audience-svg" ref="svgElement">
          <g v-for="audience in filteredAudiences" :key="audience.id">
            ...
            <g v-if="isSpecialToilet(audience.num_audiences)">
              <image 
                :x="audience.x + audience.width/2 - 12" 
                :y="audience.y + audience.height/2 - 12"
                :width="24" 
                :height="24"
                :href="getToiletIcon(audience.num_audiences)"
                class="toilet-icon"
              />
            </g>
            
            <text 
              v-if="!isSpecialToilet(audience.num_audiences)"
    ...
    <div class="three-d-container" v-if="viewMode === '3d'">
      ...
      <div class="floor-controls-3d glassmorphism">
        <button 
          ...
          :class="{ 
            'active': selectedFloor === floor, 
            'loading': isLoadingFloor && selectedFloor === floor,
            'pulse-highlight': floorsWithSearchResults.includes(floor) && searchResults.length > 1 
          }"
          @click="selectFloor(floor)"
          class="floor-btn"
...
export default {
  ...
  setup() {
    ...
    const showMultipleResultsNotification = ref(false);
    ...
    const searchResultsAudiences = computed(() => {
      if (audienceSearch.value.trim().length < 3) return [];
    ...
    const centerOnAudience = (audience) => {
      if (viewMode.value === '2d' && panzoomInstance) {
        const svgRect = svgElement.value.getBoundingClientRect();
        const mapRect = mapContent.value.getBoundingClientRect();
        
        // Вычисляем центр аудитории относительно SVG
        const centerX = audience.x + audience.width / 2;
        const centerY = audience.y + audience.height / 2;
        
        // Преобразуем координаты для panzoom
        const scale = panzoomInstance.getTransform().scale;
        const targetX = -centerX * scale + mapRect.width / 2;
        const targetY = -centerY * scale + mapRect.height / 2;
        
        // Плавное перемещение к аудитории
        panzoomInstance.smoothZoomAbs(targetX, targetY, scale);
      }
    };

    const floorsWithSearchResults = computed(() => {
      const floorsWithResults = new Set();
      searchResults.value.forEach(audience => {
        if (audience.corpus === selectedCorpus.value) {
          floorsWithResults.add(audience.floor);
        }
      });
      return Array.from(floorsWithResults);
    });

    const getResultsCountOnFloor = (floor) => {
      return searchResults.value.filter(audience => 
        audience.floor === floor && audience.corpus === selectedCorpus.value
      ).length;
    };
    ...
    const isSpecialToilet = (audienceNumber) => {
      return audienceNumber === 'Туалет Ж' || audienceNumber === 'Туалет М';
    };

    const getToiletIcon = (audienceNumber) => {
      if (audienceNumber === 'Туалет Ж') {
        return '/img/wm.png';
      } else if (audienceNumber === 'Туалет М') {
        return '/img/mn.png';
      }
      return '';
    };
    ...
    return {
      ...
      days,
      filters,
      showMultipleResultsNotification,
      currentMapImage,
      filteredAudiences,
      groupedSchedule,
...
.main-container.light {
  ...
  --card-bg: #ffffff;
  --input-bg: #ffffff;
  --audience-fill: rgba(67, 97, 238, 0);
  --label-color: #0047c1;
}
...
/* Стили для пульсирующих кнопок этажей */
.floor-btn.pulse-highlight {
